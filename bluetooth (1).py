...
# Codici da inviare sul canale di controllo BLE:
# 10  = volto felice
# 11  = volto sorpreso
# 12  = emozione sconosciuta
EMOTION_CODE = {
    "felice": 10,
    "sorpreso": 11,
    "sconosciuto": 12,
...
def rileva_emozioni(img_gray: np.ndarray , img_rgb: np.ndarray) :
    ...
    return emozioni
async def invia_emozione(client: BleakClient, emozioni: list[str]) -> None:
    """
    Prende la lista di emozioni trovate nel frame e ne manda una ad Arduino
    come singolo byte sulla caratteristica di controllo.
    """
    if not emozioni:
        # Nessun volto rilevato, non mando nulla
        return

    # Per semplicità prendo la prima emozione trovata
    emozione = emozioni[0]

    # Prendo il codice numerico, se non lo trovo uso 'sconosciuto'
    codice = EMOTION_CODE.get(emozione, EMOTION_CODE["sconosciuto"])

    try:
        # Inviamo un solo byte, esattamente come fai per il comando "1" (scatta foto)
        await client.write_gatt_char(
            CONTROL_CHAR_UUID,
            bytearray([codice]),
            response=True
        )
        print(f"[BT] -- Inviato codice emozione {codice} ({emozione}) ad Arduino")
    except BleakError as e:
        print("[ERR] -- Errore nell'invio dell'emozione:", e)
    ...
    try:
        ...
        async with BleakClient(indirizzo, timeout=20.0) as client:
            ...
            while(True):
                ...
                # Invio il risultato ad Arduino via BLE->led
                await invia_emozione(client, emozioni)

                # Mostro la camera a schermo
